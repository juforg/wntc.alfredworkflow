...
class ImgurClient(object):
    ...
    def __init__(self, client_id, client_secret, access_token=None, refresh_token=None, mashape_key=None,proxies=None):
        self.client_id = client_id
        self.client_secret = client_secret
        self.auth = None
    ...
    def make_request(self, method, route, data=None, force_anon=False):
        ...
        if method in ('delete', 'get'):
            response = method_to_call(url, headers=header, params=data, data=data, proxies=self.proxies)
        else:
            response = method_to_call(url, headers=header, data=data, proxies=self.proxies)
    ...
    def upload_from_path(self, path, config=None, anon=True):
        ...
        with open(path, 'rb') as fd:
            data = self.upload(fd, config, anon)
        return data
